...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['acao'] === 'cadastrar') {
        ...
        $stmt = $pdo->prepare("INSERT INTO enderecos (tipo_alergia) VALUES (?)");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['acao'] === 'editar') {
        ...
        $alergia = trim($_POST['tipo_alergia']);
        $stmt = $pdo->prepare("UPDATE enderecos SET tipo_alergia=? WHERE cod_alergia=?");
    ...
    if (isset($_GET['edit'])) {
        ...
        $stmt = $pdo->prepare("SELECT cod_alergia, tipo_alergia FROM enderecos WHERE cod_alergia = ?");
    ...
    if (isset($_GET['delete'])) {
        ...
        if ($ligado > 0) {
            ...
            $stmt = $pdo->prepare("DELETE FROM enderecos WHERE cod_alergia = ?");
            $stmt->execute([$id]);
    ...
    $alergias = $pdo->query("SELECT cod_alergia, tipo_alergia FROM enderecos ORDER BY tipo_alergia")->fetchAll(PDO::FETCH_ASSOC);
